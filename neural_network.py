...
class Logistic_Regression(object):
	...
	def errors(self, y_):
		correct_prediction = tf.equal( tf.argmax(y_,0), self.y_pred )
...
def sgd_optimization(minibatch_size, validation_frequency, n_epochs=100, learning_rate=.0013):
	...
	y_ = tf.placeholder("float",shape=[n_out,minibatch_size])
	...
	while epochs <= n_epochs:
		...
		for minibatch_index in xrange(num_minibatches):
			...
			for j in xrange( minibatch_index*minibatch_size , (minibatch_index+1)*minibatch_size + 1):
				...
				minibatch_images[:,j_mod_minibatch_size], minibatch_labels[:,j_mod_minibatch_size] = clean_data((j+random_start)%50000)  
	...
	return [best_W, best_b,best_cost,best_error]

val = sgd_optimization(1000,533,n_epochs=100)
